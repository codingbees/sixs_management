...
import com.ray.common.model._MappingKit2;
...
public class MainConfig extends JFinalConfig {
    ...
    public static DruidPlugin getDruidPlugin3() {
        loadConfig();
        return new DruidPlugin(p.get("jdbcUrl3"), p.get("user3"), p.get("password3"));
    }

    public static DruidPlugin getDruidPlugin4() {
        loadConfig();
    ...
    public void configPlugin(Plugins me) {
        ...
        DruidPlugin dbPlugin2 = getDruidPlugin2();
        dbPlugin2.addFilter(wallFilter);
        dbPlugin2.addFilter(new StatFilter());    // 添加 StatFilter 才会有统计数据
        dbPlugin2.setDriverClass("com.mysql.jdbc.Driver");

        //数据映射 配置ActiveRecord插件
        ActiveRecordPlugin arp2 = new ActiveRecordPlugin("rp", dbPlugin2);
        arp2.setShowSql(p.getBoolean("devMode"));
        arp2.setDialect(new MysqlDialect());
        me.add(dbPlugin2);
        me.add(arp2);
        /********添加第三个数据库*********/

        DruidPlugin dbPlugin3 = getDruidPlugin3();
        dbPlugin3.addFilter(wallFilter);
        dbPlugin3.addFilter(new StatFilter());    // 添加 StatFilter 才会有统计数据
        dbPlugin3.setDriverClass("com.mysql.jdbc.Driver");

        //数据映射 配置ActiveRecord插件
        ActiveRecordPlugin arp3 = new ActiveRecordPlugin("pt", dbPlugin3);
        arp3.setShowSql(p.getBoolean("devMode"));
        arp3.setDialect(new MysqlDialect());
        me.add(dbPlugin3);
        me.add(arp3);

        /********添加第四个数据库 ORACLE*********/

        DruidPlugin dbPlugin4 = getDruidPlugin4();
        wallFilter2 = new WallFilter();            // 加强数据库安全
        wallFilter2.setDbType(p.get("odbType"));
        dbPlugin4.addFilter(wallFilter2);
        dbPlugin4.addFilter(new StatFilter());    // 添加 StatFilter 才会有统计数据
        dbPlugin4.setDriverClass(p.get("odriver"));
        //数据映射 配置ActiveRecord插件
        ActiveRecordPlugin arp4 = new ActiveRecordPlugin("ifsapp",dbPlugin4);
        arp4.setShowSql(p.getBoolean("devMode"));
        arp4.setDialect(new MysqlDialect());
